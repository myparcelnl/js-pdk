...
export type MyParcelContext<T> = T extends ContextKey.RETURNS_FORM
  ? Context<ReturnsFormContext>
  : T extends ContextKey.SHIPMENT_LABELS
  ? Context<ShipmentLabelsContext>
  : T extends ContextKey.SHIPMENT_OPTIONS
  ? Context<ShipmentOptionsContext>
  : T extends ContextKey.SHIPPING_ADDRESS
  ? Context<ShippingAddressContext>
  : T extends ContextKey.PRINT_OPTIONS
  ? Context<PrintOptionsContext>
  : T extends ContextKey.MODULE_SETTINGS_FORM
  ? Context<ModuleSettingsFormContext>
  : T extends ContextKey.MODULE_SETTINGS_VALUES
  ? Context<ModuleSettingsValuesContext>
  : T extends ContextKey.ACCOUNT_SETTINGS
  ? Context<AccountSettingsContext>
  : T extends ContextKey.ADD_CARRIER_FORM
  ? Context<ModuleSettingsFormItem>
  : never;
