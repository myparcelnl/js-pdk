...
export type ActionResponse<CK = AdminAction> = CK extends LabelAction.DELETE
  ? ActionSuccessResponse<CK, LabelIdsData>
  : CK extends OrderAction.EXPORT
  ? ActionSuccessResponse<CK, ShipmentLabelsData>
  : CK extends OrderAction.EXPORT_PRINT
  ? ActionSuccessResponse<CK, ShipmentLabelsData & PrintData>
  : CK extends LabelAction.PRINT
  ? ActionSuccessResponse<CK, PrintData>
  : CK extends OrderAction.PRINT
  ? ActionSuccessResponse<CK, PrintData>
  : CK extends LabelAction.REFRESH
  ? ActionSuccessResponse<CK, ShipmentLabelsData>
  : CK extends OrderAction.REFRESH_LABELS
  ? ActionSuccessResponse<CK, ShipmentLabelsData>
  : never;

export const printActions = [LabelAction.PRINT, OrderAction.EXPORT_PRINT, OrderAction.PRINT] as const;
